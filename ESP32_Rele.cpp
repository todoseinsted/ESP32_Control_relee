...
constexpr char BT_DEVICE_NAME[] = "ESP32-Rele-nombre";  // Cambiar por el nombre deseado para el dispositivo Bluetooth
...
void initializeButton(ButtonState &button) {
  ...
  const bool initialReading = digitalRead(button.pin);
  button.lastReading = initialReading;
  button.stableState = initialReading;
